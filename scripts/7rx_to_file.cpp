...
int UHD_SAFE_MAIN(int argc, char *argv[]){
	...
	std::string tx_file, rx1_file, rx2_file, rx3_file, rx4_file, rx5_file, rx6_file, rx7_file, log_file;
    ...
    rx4_file += run_number;
    rx4_file += ".dat";
    rx5_file = "/root/aoa/data/rx5_";
    rx5_file += run_number;
    rx5_file += ".dat";
    rx6_file = "/root/aoa/data/rx6_";
    rx6_file += run_number;
    rx6_file += ".dat";
    rx7_file = "/root/aoa/data/rx7_";
    rx7_file += run_number;
    rx7_file += ".dat";
    ...
    uhd::usrp::multi_usrp::sptr usrp5;
    usrp5 = uhd::usrp::multi_usrp::make(ip5);
    setup_device(usrp4, rx_gain, tx_gain, freq, rate);

    uhd::usrp::multi_usrp::sptr usrp6;
    usrp6 = uhd::usrp::multi_usrp::make(ip6);
    setup_device(usrp6, rx_gain, tx_gain, freq, rate);   

    uhd::usrp::multi_usrp::sptr usrp7;
    usrp7 = uhd::usrp::multi_usrp::make(ip7);
    setup_device(usrp4, rx_gain, tx_gain, freq, rate);            
    ...
    usrp5->set_clock_config(uhd::clock_config_t::external());  
    usrp6->set_clock_config(uhd::clock_config_t::external());  
    usrp7->set_clock_config(uhd::clock_config_t::external());     
    ...
    usrp5->set_time_next_pps(uhd::time_spec_t(0.0));
    usrp6->set_time_next_pps(uhd::time_spec_t(0.0));
    usrp7->set_time_next_pps(uhd::time_spec_t(0.0));
...
*/
    ...
    usrp5->issue_stream_cmd(stream_cmd);
    usrp6->issue_stream_cmd(stream_cmd);
    usrp7->issue_stream_cmd(stream_cmd);
    ...
    std::ofstream outfile5(rx5_file.c_str(), std::ofstream::binary);
    std::ofstream outfile6(rx6_file.c_str(), std::ofstream::binary);
    std::ofstream outfile7(rx7_file.c_str(), std::ofstream::binary);
    ...
    while((not stop_signal_called) and (system_now-seconds) <= run_time){//48){
        ...
        recv_to_file<std::complex<float> >(usrp5, uhd::io_type_t::COMPLEX_FLOAT32, outfile5, spb, send_time);
        recv_to_file<std::complex<float> >(usrp6, uhd::io_type_t::COMPLEX_FLOAT32, outfile6, spb, send_time);
        recv_to_file<std::complex<float> >(usrp7, uhd::io_type_t::COMPLEX_FLOAT32, outfile7, spb, send_time);
    ...
    usrp5->issue_stream_cmd(stream_stop_cmd);
    usrp6->issue_stream_cmd(stream_stop_cmd);
	usrp7->issue_stream_cmd(stream_stop_cmd);
