    ...
    if noise_params['noise_type'] == "gaussian":
        if ndim == 2:
            ...
            # for base_idx in base_idxs:
                # col_idxs += [base_idx*(num_antennas_per_bs-1) + i for i in range(num_antennas_per_bs-1)] 
            col_idxs = [base_idx*(num_antennas_per_bs-1) + i for base_idx in base_idxs for i in range(num_antennas_per_bs-1)]
            ...
            assert False, "Maybe not implemented...double check dude..."
...
def add_angle_dependent_noise(angles, phases, ndim,  base_idxs=[-1],  noise_params={'noise_type': 'gaussian', 'k': 4, 'j': 1, 'mean': 0, 'std':0.01}):
    if noise_params['noise_type'] == "gaussian":
        if ndim == 2:
            repeated_angles = np.repeat(angles, num_antennas_per_bs-1, axis=1)
            # using 1 angle for each base station (alpha)
            col_idxs = [base_idx*(num_antennas_per_bs-1) + i for base_idx in base_idxs for i in range(num_antennas_per_bs-1)]
            gauss_noise = np.random.normal(loc=noise_params['mean'],\
                                       scale=noise_params['std'],\
                                       size=(phases.shape[0],len(col_idxs)))

            data_multiplier = nonlinear_effect_function(repeated_angles[:,col_idxs], noise_params['k'], noise_params['j'])

            gauss_noise = np.multiply(gauss_noise, data_multiplier)
            assert  gauss_noise.shape == phases[:,col_idxs].shape, "shapes don't match - whoops!"
            phases[:,col_idxs] += gauss_noise
        
        elif ndim == 3:
            assert False, "I don't know dude...might wanna check to make sure this is right before running this"
            # using 3 angles for each base station (alpha beta gamma)
            for idx in base_idxs:
                gauss_noise = np.random.normal(loc=noise_params['mean'],\
                                       scale=noise_params['std'],\
